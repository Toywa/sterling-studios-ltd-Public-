...
const FALLBACK_PROJECTS = [
  { id:'1', name:'AURORA — Wealth', client_name:'James AURORA', status:'Live', progress:100, revenue:85000, mpesa_volume:0, staging_url:'https://aurora.sterling.co.ke' },
  { id:'2', name:'LuminaPay — Fintech', client_name:'LuminaPay Ltd', status:'In Progress', progress:80, revenue:280000, mpesa_volume:12400000, staging_url:'https://luminapay.staging' },
  { id:'3', name:'SokoMart KE — Retail', client_name:'SokoMart', status:'Live', progress:100, revenue:150000, mpesa_volume:1250000, staging_url:'https://sokomart.co.ke' },
]
...
export default function Admin(){
  ...
  return (
    <div className="bg-[#F8FAFC] min-h-screen">
      <SEO title="Admin Dashboard — Sterling Studios" description="Admin dashboard" />
      <div className="max-w-7xl mx-auto px-6 lg:px-8 py-8 grid lg:grid-cols-[240px_1fr] gap-8">
        ...
        <div className="space-y-6">
          <div className="flex justify-between items-center"><h1 className="text-[28px] font-extrabold">Admin • {supabase?'Postgres Live':'Fallback Data'}</h1><div className="text-[11px] px-2 py-1 rounded-full bg-green-50 border border-green-200 text-green-700">{supabase?'Supabase Connected':'Set VITE_SUPABASE_URL to go live'}</div></div>
          ...
          <div className="bg-white rounded-[20px] border p-6">
            <div className="font-bold">Projects Table — Postgres</div>
            <div className="mt-4 space-y-2 text-[13px]">
              {projects.map(p=><div key={p.id} className="flex justify-between p-3 rounded-xl bg-slate-50 border"><span>{p.name} • {p.client_name} • {p.progress}%</span><span className="font-bold">{p.status}</span></div>)}
            </div>
          </div>
        </div>
      </div>
